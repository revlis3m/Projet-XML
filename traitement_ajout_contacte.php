...
if ($expediteurExiste && $destinataireExiste) {
    ...
    // Créer l'élément <content> avec l'attribut type="contact"
    $content = $message->addChild('content');
    $content->addAttribute('type', 'contact');

    // Ajouter la balise <texte> avec le contenu
    $texte = $content->addChild('text', $contenu);

    // Ajouter la balise <contacte> avec le nom du contact
    $contacte = $content->addChild('contacte', $nomContact);
