...
if ($expediteurExiste && $destinataireExiste) {
    ...
    // Créer l'élément <content> avec l'attribut type="file"
    $content = $message->addChild('content');
    $content->addAttribute('type', 'file');

    // Ajouter la balise <texte> avec le contenu
    $texte = $content->addChild('text', $contenu);

    // Ajouter la balise <file> avec le nom du fichier
    $file = $content->addChild('file', $nomFichier);
